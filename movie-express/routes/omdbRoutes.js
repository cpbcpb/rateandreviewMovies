...
router.post("/makeoneMovie", (req, res, next) => {
  ...
  Movie.find({ "objectFromOMDB.Title": oneMovie }).then(responseFromDB => {
    console.log("first then of the post route to save movie", responseFromDB);
    if (
      responseFromDB !== null &&
      responseFromDB !== undefined &&
      responseFromDB.length !== 0
    ) {
      console.log("the if statement of the post route to save movie");
      res
        .status(304)
        .json({ message: "movie already exists in your DataBase" });
      return;
    } else {
      ...
      axios
        ...
        .then(thing => {
          ...
          setTimeout(function() {
            ...
            res.status(200).json({
              message: "new movie ha been added to DataBase",
              newMovie
            });
          }, 1200);
        })
